$(document).ready(function(){$('.js-phonemask-input').each(function(){var $this=$(this);t_form_phonemask_load($this)})});function t_form_phonemask_load_one(recid){$('#rec'+recid).find('.js-phonemask-input').each(function(){var $this=$(this);t_form_phonemask_load($this)})}
function t_form_phonemask_load($this){var id=$this.attr('data-phonemask-id');var lid=$this.attr('data-phonemask-lid');var maskcountry=$this.attr('data-phonemask-maskcountry')||'';if(!maskcountry||maskcountry.length==0){maskcountry=''}
$this.attr('data-phonemask-init','yes');setTimeout(function(){try{t_form_phonemask_init(id,lid,maskcountry)}catch(err){console.log(err)}},1000)}
function t_form_phonemask_init(recid,lid,iso){if(typeof iso!='undefined'&&iso!=''){t_form_phonemask_do(recid,lid,iso)}else{if(typeof window.geo_iso!="undefined"&&window.geo_iso!=""&&window.geo_iso!=null){t_form_phonemask_do(recid,lid,window.geo_iso)}else{$.ajax({type:'GET',url:'https://geo.tildacdn.com/geo/country/',success:function(data){window.geo_iso=data;t_form_phonemask_do(recid,lid,data)},error:function(){t_form_phonemask_do(recid,lid,'')},timeout:1000*5})}}}
function t_form_phonemask_do(recid,lid,iso){var inputGroup=$('#rec'+recid).find('[data-input-lid="'+lid+'"]');if(inputGroup.length==0){if(typeof window.tildamode!='undefined'&&window.tildamode=='zero'){inputGroup=$('.tn-artboard').find('[data-input-lid="'+lid+'"]');if(inputGroup.length==0){inputGroup=$('.tn-artboard')}}else{inputGroup=$('body').find('[data-input-lid="'+lid+'"]');if(inputGroup.length==0){inputGroup=$('body')}}}
t_form_phonemask_replace_input(inputGroup);if(!$('.t-input-phonemask__wrap').hasClass('t-input-phonemask__wrap-style')){t_form_phonemask_add_style()}
var input=inputGroup.find('.t-input-phonemask');if(typeof iso!='string'||iso==null){console.log('Error. ISO is not string or null: '+iso);iso=''}
if(iso.length>0){iso=iso.toLowerCase()}
if(iso.length>2){console.log('Error ISO: '+iso);iso=''}
var countries=t_form_phonemask_getCountriesList();if(typeof countries[iso]!='undefined'){var country=countries[iso]}else{var country={'n':'Russian Federation (Российская Федерация)','c':'+7','m':'+7(999) 999-99-99'}
var iso='ru'}
input.attr('data-phonemask-code',iso.length>0?country.c:'');input.attr('data-phonemask-mask',iso.length>0?country.m:'');t_form_phonemask_calc_maxlength(input);if(iso.length>0){localStorage.setItem('iso_code',iso);inputGroup.find('.t-input-phonemask__select-flag').attr('data-phonemask-flag',iso);inputGroup.find('.t-input-phonemask__select-code').html(country.c);if(typeof input.attr('data-phonemask-mask')!='undefined'){input.attr('placeholder',input.attr('data-phonemask-mask').substr(input.attr('data-phonemask-code').length).replace(/^-+/,''))}
t_form_phonemask_change_min_length(input.attr('placeholder'),input.attr('data-phonemask-code'),inputGroup)}
t_form_phonemask_add_current_style(inputGroup)
inputGroup.find('.t-input-phonemask__options-wrap').html(t_form_phonemask_draw_selector(countries));inputGroup.find('.t-input-phonemask__select').on('click',function(){$('.t-input-phonemask__options-wrap').removeClass('t-input-phonemask__options-wrap_open')
$(this).siblings('.t-input-phonemask__options-wrap').toggleClass('t-input-phonemask__options-wrap_open')});inputGroup.find('.t-input-phonemask__options-item').on('click',function(){t_form_phonemask_choose_country(inputGroup,lid,$(this));t_form_phonemask_calc_maxlength(input)});input.on('input',function(){var $this=$(this);$this.attr('data-phonemask-current',$this.val());var curValue=t_form_phonemask_copypaste_handling($this,input.attr('data-phonemask-without-code'));t_form_phonemask_add_number_mask(curValue,$(this));t_form_phonemask_change_val(input,inputGroup)});document.onmouseup=function(e){var container=$('.t-input-phonemask__options-wrap');if(e.target.closest('.t-input-phonemask__options-wrap')===null&&e.target.closest('.t-input-phonemask__select')===null){container.removeClass('t-input-phonemask__options-wrap_open')}}
t_form_phonemask_change_val(input,inputGroup);var word='';var timeout='';function resetTimeout(){clearTimeout(timeout);timeout=setTimeout(function(){word=''},1500)}
$(document).on('keyup',function(e){$('.t-input-phonemask__options-wrap').each(function(){if($(this).hasClass('t-input-phonemask__options-wrap_open')){var searchCountry=[];$('.t-input-phonemask__options-item').removeClass('t-input-phonemask__options-item_chosen');if(e.keyCode>=48&&e.keyCode<=57||e.keyCode>=96&&e.keyCode<=105){word=word+e.key;for(var country in countries){t_form_phonemask_search_country($(this),word,t_form_phonemask_regexp_replace(word),searchCountry,countries,country,countries[country].c)}}else if(e.keyCode>=65&&e.keyCode<=90||e.keyCode==32){word=word+e.key;for(var country in countries){t_form_phonemask_search_country($(this),word,'^'+t_form_phonemask_regexp_replace(word),searchCountry,countries,country,countries[country].n)}}
resetTimeout()}})})}
function t_form_phonemask_calc_maxlength(input){var maskWithoutCode=input.attr('data-phonemask-mask').substr(input.attr('data-phonemask-code').length);if(maskWithoutCode.charAt(0)=='-'){maskWithoutCode=maskWithoutCode.substring(1)}
input.attr('maxlength',maskWithoutCode.length);input.attr('data-phonemask-without-code',maskWithoutCode)}
function t_form_phonemask_copypaste_handling($this,maskWithoutCode){var currentValue=$this.val();var valueArr=currentValue.match(/[0-9]/g)!=null?currentValue.match(/[0-9]/g):currentValue;if(valueArr.length>maskWithoutCode.match(/[0-9]/g).join('').length){currentValue=valueArr.splice($this.attr('data-phonemask-code').match(/[0-9]/g).length).join('')}
return currentValue}
function t_form_phonemask_regexp_replace(word){var regexp=word.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,'\\$&');return regexp}
function t_form_phonemask_search_country($this,word,regexp,searchCountry,countries,country,property){var regExp=new RegExp(regexp,'i');if(property.search(regExp)!=-1){searchCountry.push($this.find('#'+countries[country].id+''));searchCountry[0].addClass('t-input-phonemask__options-item_chosen');var container=$this;container.animate({scrollTop:$this.find('.t-input-phonemask__options-item_chosen').offset().top-container.offset().top+container.scrollTop()},0)}}
function t_form_phonemask_replace_input(inputGroup){var inputTel=inputGroup.find('.js-phonemask-input');var name=inputTel.attr('name');var dataReq='';if(typeof inputTel.attr('data-tilda-req')!='undefined'){dataReq='data-tilda-req="1"'}
var borderBottom=!1;if(inputTel.hasClass('t-input_bbonly')){var borderBottom=!0}
var style=inputTel.attr('style');inputTel.replaceWith('<div class="t-input t-input-phonemask__wrap" style="'+style+'">\
          ...
          <div class="t-input-phonemask__options-wrap"></div>\
    ...
    </div>');if(borderBottom){inputGroup.find('.t-input-phonemask__wrap').addClass('t-input_bbonly')}
inputGroup.find('.t-input-phonemask__wrap').siblings('.t-input__vis-ph').remove()}
function t_form_phonemask_draw_selector(countries){var str='';for(var country in countries){str+='<div class="t-input-phonemask__options-item" id="'+countries[country].id+'" data-phonemask-name="'+countries[country].n+'" data-phonemask-mask="'+countries[country].m+'" data-phonemask-country-code="'+country+'" data-phonemask-code="'+countries[country].c+'">';str+='<div class="t-input-phonemask__options-name">'+countries[country].n+'</div>';str+='<span class="t-input-phonemask__options-code">'+countries[country].c+'</span>';str+='<span class="t-input-phonemask__options-flag t-input-phonemask__options-flag_'+country+'"></span>';str+='</div>'}
return str}
function t_form_phonemask_add_number_mask(val,$this){var code=$this.attr('data-phonemask-code');var mask=$this.attr('data-phonemask-mask').substr(code.length);var number=val.match(/\d+/g);if(number){number=number.join('')}else{number=''}
var numberArr=number.split('');var regex=/(\+|\d+|[\s()-]|9+)/g;var maskArr=mask.match(regex);var phoneString='';if(maskArr!=null){for(var i=0;i<maskArr.length;i++){if(maskArr[i][0]=='9'){phoneString+=numberArr.slice(0,maskArr[i].length).join('');numberArr.splice(0,maskArr[i].length)}else{phoneString+=maskArr[i]}}
phoneString=phoneString.replace(/[\s()-]+$/,'');phoneString=phoneString.replace(/^-+/,'');$this.val(phoneString)}}
function t_form_phonemask_choose_country(inputGroup,lid,$this){var input=inputGroup.find('.t-input-phonemask');var select=inputGroup.find('.t-input-phonemask__select');var currentCode=$this.attr('data-phonemask-code');var currentMask=$this.attr('data-phonemask-mask');inputGroup.find('.t-input-phonemask__options-wrap').removeClass('t-input-phonemask__options-wrap_open');input.attr('data-phonemask-code',currentCode);input.attr('data-phonemask-mask',currentMask);input.attr('placeholder',currentMask.substr(currentCode.length).replace(/^-+/,''));select.find('.t-input-phonemask__select-code').html(currentCode);select.find('.t-input-phonemask__select-flag').attr('data-phonemask-flag',$this.attr('data-phonemask-country-code'));localStorage.setItem('iso_code',$this.attr('data-phonemask-country-code'));input.focus();if(input.attr('data-phonemask-current')){t_form_phonemask_add_number_mask(input.attr('data-phonemask-current'),input)}
t_form_phonemask_change_val(input,inputGroup);t_form_phonemask_change_min_length(input.attr('placeholder'),currentCode,inputGroup)}
function t_form_phonemask_add_current_style(inputGroup){var optionsWrap=inputGroup.find('.t-input-phonemask__options-wrap');optionsWrap.parents('.t-input-block').css('overflow','initial');setTimeout(function(){optionsWrap.parents('.t-input-block').css('overflow','initial')},2000);inputGroup.find('.t-input-phonemask').css('color',optionsWrap.css('color'));inputGroup.find('.t-input-phonemask').css('font-size',optionsWrap.css('font-size'));inputGroup.find('.t-input-phonemask__select-code').css('font-size',optionsWrap.css('font-size'));inputGroup.find('.t-input-phonemask').css('font-weight',optionsWrap.css('font-weight'))}
function t_form_phonemask_change_val(input,inputGroup){if(typeof input.val()!='undefined'){if(input.val().length!==0){inputGroup.find('.js-phonemask-result').val(input.attr('data-phonemask-code')+' '+input.val())}}}
function t_form_phonemask_change_min_length(mask,code,inputGroup){if(typeof code!='undefined'&&typeof mask!='undefined'){var spaceLength=1;var floatingLength=1;var phonemaskCode=inputGroup.find('.js-phonemask-result').siblings('input.t-input').attr('data-phonemask-code');if(phonemaskCode=='+972'||phonemaskCode=='+49'||phonemaskCode=='+372'||phonemaskCode=='+355'||phonemaskCode=='+213'||phonemaskCode=='+975'||phonemaskCode=='+267'||phonemaskCode=='+359'||phonemaskCode=='+53'||phonemaskCode=='+36'||phonemaskCode=='+961'){inputGroup.find('.js-phonemask-result').attr('data-tilda-rule-minlength',code.length+spaceLength+mask.length-floatingLength)}else{inputGroup.find('.js-phonemask-result').attr('data-tilda-rule-minlength',code.length+spaceLength+mask.length)}}else{inputGroup.find('.js-phonemask-result').attr('data-tilda-rule-minlength',1)}}
